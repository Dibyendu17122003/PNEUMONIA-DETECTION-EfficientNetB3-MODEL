...
def safe_speak(text: str):
    ...
    try:
        ...
        st.toast("Voice output unavailable on this system.", icon="⚠")
...
def send_email_with_attachment(sender_email: str, app_password: str, recipient: str, subject: str, body: str, attachment_bytes: bytes, filename: str) -> bool:
    ...
    try:
        if not (sender_email and app_password and recipient):
            st.warning("Please fill Sender Gmail, App Password and Recipient.", icon="⚠")
            return False
        ...
        st.toast("Email sending failed. Check Gmail & App Password.", icon="⚠")
        return False
        ...
        with st.spinner("Analyzing X-ray with EfficientNet…"):
            try:
                ...
                st.markdown(f"<div class='{style}'>"
                            f"{'⚠' if label=='Pneumonia' else '✅'} <b>{label}</b><br>"
                ...
                if enable_gradcam:
                    try:
                        ...
                        st.toast("Grad-CAM failed on this model/TF build.", icon="⚠")
...
if send_btn:
    if st.session_state.last_pdf_bytes is None:
        st.warning("Generate a report first (run a prediction).", icon="⚠")
    else:
...
st.markdown("<p class='small-muted'>⚠ This tool is for educational use only and is not a medical device. "
